...
class Image extends Entity
{
    ...
    {
        $model = $this->_properties['model'];
        $entity_id = $this->_properties['entity_id'];

        if( $this->_properties['model'] == 'Blocks' ){

            $BlockGroups = TableRegistry::get('BlockGroups');

            if( $blockGroup = $BlockGroups->findById( $this->_properties['entity_id'] )->first() ){
                $Table = TableRegistry::get($blockGroup->model);
