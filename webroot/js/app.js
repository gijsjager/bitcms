...
!function () {
    ...
    function monochromatic(color, results) {
        results = results || 6;
        for (var hsv = tinycolor(color).toHsv(), h = hsv.h, s = hsv.s, v = hsv.v, ret = [],
                 modification = 1 / results; results--;)ret.push(tinycolor({
            h: h,
            s: s,
            v: v
        })), v = (v + modification) % 1;
...
!function (a) {
    a.isBreakpoint = function (b) {
        var c, d;
        return c = a("<div/>", {class: "hidden-" + b + "-down"}).appendTo("body"), d = c.is(":hidden"), c.remove(), d
    }, a.extend(a, {
        isXs: function () {
