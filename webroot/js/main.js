...
function Forms(){
    ...
    jQuery('form').submit(function(e){
        ...
        b.prop('disabled', true).attr('data-backup', b.val()).val('Bezig...');

        // validate
        f.find('[required]').each(function(){
            if( jQuery(this).val() == '' ){
                jQuery(this).addClass('error animated shake');
                e = true;
            }
        });

        b.val( b.data('backup') ).prop('disabled', false);
        ...
        if( e === false ){
            jQuery.ajax({
                ...
                if( r == 'OK' ){
                    alert('Het bericht is verstuurd! Dank je wel!');
                } else {
                    alert(r);
                }
            });
...
function setHumnzr(elm){
    ...
    if( !f.find('#hmnzr').length ){
        ...
        var val = 'Gijs123DFSk42' + yyyy + mm + dd;
        
        f.append('<input type="hidden" name="humnzr" id="hmnzr" value="'+val+'" required />');
    }
}
